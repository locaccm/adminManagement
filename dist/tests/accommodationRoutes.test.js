...
describe("Accommodation Routes", () => {
    describe("GET /accommodations", () => {
        it("should return 400 if userId is missing", async () => {
            const res = await (0, supertest_1.default)(index_1.default).get("/accommodations");
            expect(res.status).toBe(400);
            expect(res.body).toHaveProperty("error", "userId is required");
        });
        it("should return accommodations for a given userId", async () => {
            const res = await (0, supertest_1.default)(index_1.default).get("/accommodations?userId=1");
            expect([200, 500]).toContain(res.status);
        });
    });
    describe("POST /accommodations", () => {
        it("should create a new accommodation", async () => {
            const res = await (0, supertest_1.default)(index_1.default).post("/accommodations").send({
    ...
    describe("PUT /accommodations/:id", () => {
        it("should update an accommodation", async () => {
            const res = await (0, supertest_1.default)(index_1.default).put("/accommodations/1").send({
    ...
    describe("DELETE /accommodations/:id", () => {
        it("should delete an accommodation", async () => {
            const res = await (0, supertest_1.default)(index_1.default).delete("/accommodations/1");
            expect([200, 404, 500]).toContain(res.status);
        });
    });
});
