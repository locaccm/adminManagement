...
describe('accommodationController', () => {
  ...
  test('getAccommodations - success', async () => {
    ...
    (prisma.accommodation.findMany as jest.Mock).mockResolvedValue([{ ACCN_ID: 1 }]);

    await accommodationController.getAccommodations(req, res);
    expect(prisma.accommodation.findMany).toHaveBeenCalledWith({ where: { USEN_ID: 1 } });
  ...
  test('createAccommodation - success', async () => {
    ...
    (prisma.accommodation.create as jest.Mock).mockResolvedValue(fakeAccommodation);

    await accommodationController.createAccommodation(req, res);
    expect(prisma.accommodation.create).toHaveBeenCalledWith({ data: req.body });
